...
void dispBar(int health, int coins, int ammo, int maxAmmo, char gun[]) {
    // Top border (120 chars total)
    cout << blue << bold << char(201);
    for (int i = 0; i < 118; i++) cout << char(205);
    cout << char(187) << endl << reset;
    
    // Content line - starts with left border
    cout << blue << bold << char(186) << reset;
    
    // HEALTH SECTION (30 chars total)
    cout << " HEALTH: " << red << bold;
    // Health bars (10 blocks)
    int healthBars = health / 10;
    if (healthBars > 10) { healthBars = 10; }
    for (int i = 0; i < healthBars; i++) { cout << char(219); }
    for (int i = healthBars; i < 10; i++) { cout << " "; }
    cout << reset << " " << health << "%";


    // Pad to exactly 30 chars
    int healthPad = 30 - 10 - 8; // 10 blocks + " HEALTH: " (8)
    if (health >= 100) { healthPad -= 4; } // 3 digits + %
    else if (health >= 10) { healthPad -= 3; } // 2 digits + %
    else { healthPad -= 2; } // 1 digit + %
    for (int i = 0; i < healthPad; i++) { cout << " "; }
    


    // COINS SECTION (20 chars total)
    cout << blue << bold << char(186) << reset << " COINS: " << yellow << bold << coins << reset;
    // Pad coins section
    int coinsPad = 20 - 7; // " COINS: " = 7 chars
    if (coins >= 100) { coinsPad -= 3; }
    else if (coins >= 10) { coinsPad -= 2; }
    else { coinsPad -= 1; }
    for (int i = 0; i < coinsPad; i++) { cout << " "; }
    


    // WEAPON SECTION (30 chars total)
    cout << blue << bold << char(186) << reset << " WEAPON: " << yellow << bold << gun << reset;
    // Calculate gun name length
    int gunLen = 0;
    while (gun[gunLen] != '\0') { gunLen++; }
    // Pad weapon section
    int weaponPad = 30 - 8 - gunLen; // " WEAPON: " = 8
    for (int i = 0; i < weaponPad; i++) { cout << " "; }
    

    
    // AMMO SECTION (30 chars total)
    cout << blue << bold << char(186) << reset << " AMMO: " << yellow << bold << ammo << reset << "/" << yellow << maxAmmo << reset;
    // Calculate ammo digits
    int ammoDigits = 1;
    if (ammo >= 10) { ammoDigits = 2; }
    if (ammo >= 100) { ammoDigits = 3; }
    int maxAmmoDigits = 1;
    if (maxAmmo >= 10) { maxAmmoDigits = 2; }
    if (maxAmmo >= 100) { maxAmmoDigits = 3; }
    // Pad ammo section
    int ammoPad = 30 - 7 - ammoDigits - 1 - maxAmmoDigits; // " AMMO: " = 7
    for (int i = 0; i <= ammoPad; i++) { cout << " "; }
    
    // Right border
    cout << blue << bold << char(186) << reset << endl;
    
    // Bottom border (120 chars total)
    cout << blue << bold << char(200);
    for (int i = 0; i < 118; i++) { cout << char(205); }
    cout << char(188) << endl << reset;
}
